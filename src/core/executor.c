...
result_t *executor_run_fault(const fault_def_t *f, const char *op, const params_t *p, int timeout_ms) {
    ...
    if (g_mock) {
        return g_mock(f->script, env);  /* mock 场景 env 供测试检查，不释�?*/
    }
    ...
    if (pid == 0) {
        apply_env(env, nenv);
        /* stdin �?/dev/null 读：脚本的参数经 env 传入，不应读 dcat �?stdin�?         * 非交互场景（cron/ctest/管道）下 stdin 为空管道，脚本误 read 会永久阻塞�?*/
...
int executor_run_raw_fault(const fault_def_t *f, const char *op, const params_t *p) {
    ...
    if (g_mock) {
        ...
        int code = r ? r->code : 1;
        result_free(r);
        return code;  /* mock 场景 env 供测试检查，不释�?*/
    }
    clear_stale_env_params(f);
    apply_env(env, nenv);
