...
int test_cores_parse_list_range_mixed_single(void) {
    ...
    ASSERT_TRUE(bitset(b, 0) && bitset(b, 1) && !bitset(b, 2) && !bitset(b, 3) && bitset(b, 4) && bitset(b, 5) && bitset(b, 6) && !bitset(b, 7));
...
int test_reinject_different_cores_concurrent_ok(void) {
    ...
    params_t p2;
    params_init(&p2);
    params_set(&p2, "cores", "2,3");
    result_t *r2 = dispatch_route_force("rCPU_overload", "inject", &p2, 0);
    ASSERT_TRUE(r2 && r2->code == 0);
    result_free(r2);
...
int test_reinject_network_scalar_reject(void) {
    ...
    params_set(&p1, "delay_ms", "100");
    ASSERT_TRUE(dispatch_route_force("rNET_delay", "inject", &p1, 0)->code == 0);

    params_t p2;
    params_init(&p2);
    params_set(&p2, "iface", "eth0");
    params_set(&p2, "delay_ms", "200");
    ...
    params_t p3;
    params_init(&p3);
    params_set(&p3, "iface", "eth1");
    params_set(&p3, "delay_ms", "200");
    result_t *r3 = dispatch_route_force("rNET_delay", "inject", &p3, 0);
    ASSERT_TRUE(r3 && r3->code == 0);
    result_free(r3);
