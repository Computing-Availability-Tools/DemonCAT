...
/* NULL 防御 + 单次求值（避免带副作用参数二次求值）；失�?return 1 中断当前测试而非崩进�?*/
#define ASSERT_TRUE(x) do { if (!(x)) { fprintf(stderr, "ASSERT_TRUE fail: %s:%d\n", __FILE__, __LINE__); return 1; } } while (0)
#define ASSERT_INT_EQ(a, b) do { long long _ia = (long long)(a); long long _ib = (long long)(b); if (_ia != _ib) { fprintf(stderr, "INT_EQ fail: %lld != %lld at %s:%d\n", _ia, _ib, __FILE__, __LINE__); return 1; } } while (0)
