...
/* Build a success result. `data` (cJSON*) is consumed (freed); pass NULL for empty {}. */
result_t *result_ok(const char *op, const char *uid, cJSON *data);

/* Build an error result. `code` is a DCAT_E_* constant (also exit code). */
result_t *result_err(const char *op, const char *uid, int code, const char *msg);

/* Print result JSON to stdout (trailing newline). NULL-safe (prints error JSON). */
