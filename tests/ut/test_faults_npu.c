...
int main(void) {
    ...
    /* rNPU_arp (action=add) */
    {
        ...
        result_free(r);
        r = dispatch_route("rNPU_arp", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rNPU_arp (action=del) */
    ...
    /* rNPU_route (action=add) */
    {
        ...
        r = dispatch_route("rNPU_route", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rNPU_iprule (action=add) */
    {
        ...
        result_free(r);
        r = dispatch_route("rNPU_iprule", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rNPU_iproute (action=add, 7 params — use params_set, mkparams max=6) */
    {
        ...
        params_init(&p);
        params_set(&p, "chip", "0");
        params_set(&p, "action", "add");
        params_set(&p, "ip", "10.0.0.1");
        params_set(&p, "ip_mask", "255.0.0.0");
        ...
        check_param_env("table", "100");
        result_free(r);
        r = dispatch_route("rNPU_iproute", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
