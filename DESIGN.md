...
- [3. 模块职责](#3-模块职责)
  - [3.1 main.c](#31-mainc)
  - [3.2 cli.c（递归下降解析器）](#32-clic递归下降解析器)
...
- [4. 故障详设（按模块分组）](#4-故障详设按模块分组)
  - [4.1 网络模块（network）](#41-网络模块network10-条新增--已有-rnet_delay)
  - [4.2 进程模块（process）](#42-进程模块process4-条)
  - [4.3 CPU 模块（cpu）](#43-cpu-模块cpu1-条--已有-rcpu_overload)
  - [4.4 存储模块（storage）](#44-存储模块storage1-条)
  - [4.5 NPU 模块（npu，v0.3 新增）](#45-npu-模块npu20-条v03-新增)
- [5. 关键流程](#5-关键流程)
...
```
  ...
  registry_find(uid)  ──→  fault_def_t *  (cnf 命中)
   ...
   │              └─ 可恢复:
   │                    cnf:    executor_run(script, DCAT_OP=inject) → state_add(uid)
   │                    注入器:  inj->inject(params)                    → state_add(uid)
   │                    → output_ok(message, record_id)
   ├── clean ──→ precheck(op∈supported_ops) → state_find
   │              ├─ 无记录 → output_err(1, "no active injection")
   │              └─ 记录:
   │                    cnf:    executor_run(script, DCAT_OP=clean, 回放 where)
...
typedef struct {
    ...
    char optional_params[128];   /* "speed_mbps" — 可选参数名, 不含 duration */
} fault_def_t;
...
> `fault_def_t` 不含 `safety` / `timeout` 字段（本期不实现安全确认、超时自动恢复）。预检只校验 `required_params`，`optional_params` 缺省时不报错。`inject`-only 故障不创建 `injection_record_t`。`injection_record_t` 不含 `bg_pid`（统一同步执行，dcat 不托管子进程 pid）。
...
### 3.1 main.c
读取配置（`config_load`，固定路径见 SPEC §7）→ 注册 `fault_def` 表到 registry → 读取 argv（单条命令串或 `--config` / `--help`）→ `cli_parse` → `registry_find`（未命中回退 `injector_find`，见 §7.4）→ 按 op 分发（`dispatch_route`）→ `output_print` → 返回退出码。
...
### 3.5 precheck.c
- `precheck(fault_def, op, params)`：执行 SPEC §4.2 预检 5 步；返回 `result_t`（成功 / 带错误码）。`inject`-only 故障跳过第 5 步（无并发约束）。
- 本模块只做静态校验（uid 存在、op 合法、参数齐全、脚本可执行、无并发），**不做交互确认**（本期不实现 safety_confirm）。
- 注入器故障的预检由 `injector_t->precheck` 函数指针实现（§7.3），不走本模块。

### 3.6 state.c
- 静态 `injection_record_t g_records[DCAT_MAX_RECORDS]` + `pthread_mutex_t`。
- `state_add(uid)` → 分配 `record_id`，写 `started_at` / `active=1`。**仅 `inject,clean,query` 故障调用**（cnf 与注入器均如此）。
- `state_find(uid)` / `state_find_by_id(id)` / `state_list()`。
- `state_mark_inactive(id)`：clean 后置 `active=0`。
- 持久化：变更后写 `~/.demoncat/state.json`（cJSON 序列化），启动时加载（恢复 `record_id` 计数与未清理记录）。
...
- `dispatch_clean_record(rec)`：重跑脚本 `DCAT_OP=clean`（回放原 `where` 参数）或调 `inj->clean(params)`。由 `dispatch_clean` 调用。**无 background→executor_kill 分支**（本期统一同步执行，clean 始终由脚本/注入器自行清理其子进程与系统资源）。
...
clean = dcat 重跑脚本 `DCAT_OP=clean`（回放原 `where` 参数）。脚本据此删 qdisc / 删 iptables 规则 / 起服务 / 读 pidfile kill 子进程。

#### 4.1.2 10 条新增故障差异表

| UID | 必填参数 | 可选参数 | clean 机制 | 备注 |
|---|---|---|---|---|
| `rNET_delay` * | iface,delay_ms | — | `tc qdisc del` | v0.1 已实现示例 |
...
parse → registry_find(uid)
  ├─ cnf 命中 fault_def:
  │     → precheck(op∈supported_ops, required_params 齐全, 脚本可执行, [可恢复:无活跃])
  │     → dispatch:
  │        ├─ inject-only:    executor_run(script, DCAT_OP=inject) → output_ok (无 state)
  │        └─ 可恢复:          executor_run(script, DCAT_OP=inject) → state_add(uid)
  │                          → output_ok(message, record_id)
  ...
  │     │        └─ 可恢复:       inj->inject(params) → state_add(uid)
  │     │                          → output_ok(message, record_id)
...
parse → registry_find(uid)
  ...
  │     → state_find(uid)
  │        ├─ 无记录 → output_err(1, "no active injection")
  │        └─ 记录 → executor_run(script, DCAT_OP=clean, 回放原 where 参数)
  │        → state_mark_inactive → output_ok
...
- **query（有 uid）**：验证故障是否真的生效。用户 `where` 参数传入（与 inject 参数独立）。
...
- clean 时 dcat 重跑同一脚本 `DCAT_OP=clean`，并回放原 `where` 参数（若有）。脚本读取 pidfile/sidecar，kill 子进程后清理资源（删 qdisc / 删 iptables 规则 / 起服务 / kill 子进程）。脚本退出码非 0 时 dcat 报错且**不 mark inactive**（故障可能仍在系统上）。
...
typedef struct injector_t {
    ...
    /* 预检：op∈{inject,clean,query}。执行 SPEC §4.2 预检 5 步的等价校验。
...
| `precheck` | SPEC §4.2 预检 5 步 | 成功：`result_ok`；失败：`result_err(code, msg)` |
...
│   └── scripts/
│       ├── cpu/                    # v0.1 示例 + v0.2 (1 脚本)
│       │   ├── cpu_overload.sh        # v0.1 示例
│       │   └── cpu_core_offline.sh    # v0.2
│       ├── network/                # v0.1 示例 + v0.2 (10 新增脚本)
│       │   ├── net_delay.sh           # v0.1 示例
│       │   ├── net_loss.sh
│       │   ├── net_reorder.sh
│       │   ├── net_down.sh
│       │   ├── net_degrade.sh
...
```
dcat "<command> <uid> [params]" [--config <path>] [--help]
```
...
dcat "inject rCPU_overload (cores) values (4)"
...
- **决策7（发布批次）**：v0.1 / v0.2 / v0.3+ 批次式持续扩充（SPEC §8）。
