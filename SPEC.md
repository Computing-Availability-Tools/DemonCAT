...
### 3.3 目录清单（共 18 条：v0.1 已有 2 + v0.2 新增 16）

> 标 `*` 为 v0.1 已实现示例；其余 16 条为 v0.2 新增（需求"网络包延时"对应 `rNET_delay`，已在 v0.1 实现）。完整 cnf 声明见 §7。
...
- **新增模块**：在 `module` 字段取新值（如 `memory` / `npu`），脚本放到 `config/scripts/<module>/`，cnf 加段即可。
...
| **v0.2（本期）** | 需求.md 17 项：network 11（含 rNET_delay 已 v0.1 实现）+ process 4 + cpu 1 + storage 1；新增 16 UID | ✅ 已完成 |
| **v0.3+** | 持续按需扩充，不预设模块顺序；可新增模块（memory / npu / …）或在现有模块内加故障 | 📋 后续 |
...
| 表驱动故障 | 18 故障的 inject/clean/query 下发命令串 + env | CTest + mock_executor | test_faults（通用）+ test_faults_network / test_faults_process / test_faults_cpu_storage（按模块） |
...
| 可测 | mock_executor + 表驱动；无硬件可测全部 18 故障的下发命令串 |
