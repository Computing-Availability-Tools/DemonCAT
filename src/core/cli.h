...
typedef struct {
    ...
    int port;            /* --port <n> (仅 serve); 0=默认 8080 */
    const char *bind;    /* --bind <addr> (仅 serve); NULL=0.0.0.0 (全接口) */
    const char *webroot; /* --webroot <dir> (仅 serve); NULL=派生 */
    int allow_write;     /* --allow-write (仅 serve); 0=只读默认,1=开 POST inject/clean */
