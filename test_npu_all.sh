...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  NPU 19 条故障完整测试 (inject → query → clean → query)      ║"
...
$D clean rNPU_ip_change --chip=2 --address=10.20.10.100 --netmask=255.255.255.0
$D query  rNPU_ip_change --chip=2
...
$D clean rNPU_gw_change --chip=2 --gateway=10.20.10.254
$D query  rNPU_gw_change --chip=2
...
$D clean rNPU_netdetect_change --chip=2 --address=10.20.10.254
$D query  rNPU_netdetect_change --chip=2
...
$D query  rNPU_arp_poison --chip=2
$D clean rNPU_arp_poison --chip=2 --dev=eth0 --ip=10.20.10.200 --mac=de:ad:be:ef:00:01
$D query  rNPU_arp_poison --chip=2
...
$D query  rNPU_route_add --chip=2
$D clean rNPU_route_add --chip=2 --address=10.30.0.0 --netmask=255.255.255.0 --gateway=10.20.10.1
$D query  rNPU_route_add --chip=2
...
$D query  rNPU_iprule_add --chip=2
$D clean rNPU_iprule_add --chip=2 --dir=from --ip=10.20.10.0 --table=100
$D query  rNPU_iprule_add --chip=2
...
$D query  rNPU_iprule_del --chip=2
...
$D query  rNPU_iproute_add --chip=2 --table=0
$D clean rNPU_iproute_add --chip=2 --ip=10.40.0.0 --ip_mask=24 --via=10.20.10.1 --dev=eth0 --table=0
$D query  rNPU_iproute_add --chip=2 --table=0
...
$D query  rNPU_iproute_del --chip=2 --table=0
...
$D clean rNPU_bw_limit --chip=2 --bw_limit=50000
$D query  rNPU_bw_limit --chip=2
...
$D clean rNPU_mtu_mismatch --chip=2 --size=1500
$D query  rNPU_mtu_mismatch --chip=2
...
$D clean rNPU_dscp_tc_change --chip=2 --dscp=10 --tc=2
$D query  rNPU_dscp_tc_change --chip=2 --dscp=10
...
$D clean rNPU_prio_tc_change --chip=2 --map=0,0,0,0,1,1,1,1
$D query  rNPU_prio_tc_change --chip=2
...
$D clean rNPU_pfc_change --chip=2 --bitmap=0,0,0,0,1,0,0,0
$D query  rNPU_pfc_change --chip=2
...
$D clean rNPU_roce_port_change --chip=2 --port=45000
$D query  rNPU_roce_port_change --chip=2

# ---------- 额外: query 不带参数遍历所有 device ----------
echo ""; echo "========== 额外: query 不带参数 (遍历所有 device) =========="
rm -f ~/.demoncat/state.json
echo "--- rNPU_link_down ---"
$D query rNPU_link_down
echo "--- rNPU_bw_limit ---"
$D query rNPU_bw_limit
echo "--- rNPU_mtu_mismatch ---"
