...
case "${DCAT_OP:-inject}" in
    inject)
        ...
        SIDECAR="/tmp/dcat-rNET_bw_limit-${iface}.sidecar"
        tc qdisc add dev "$iface" root tbf rate "${rate}kbit" burst 32kbit latency 400ms || { echo "$iface 已有 root qdisc ($(tc qdisc show dev "$iface" 2>/dev/null | grep root | head -1))" >&2; echo "请先清理: dcat clean --all �?tc qdisc del dev $iface root" >&2; exit 1; }
        echo "$iface" > "$SIDECAR"
