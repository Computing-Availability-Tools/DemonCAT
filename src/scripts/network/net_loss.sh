...
case "${DCAT_OP:-inject}" in
    inject)
        ...
        SIDECAR="/tmp/dcat-rNET_loss-${iface}.sidecar"
        tc qdisc add dev "$iface" root netem loss random "${pct}%" || { echo "$iface 已有 root qdisc ($(tc qdisc show dev "$iface" 2>/dev/null | grep root | head -1))" >&2; echo "请先清理: dcat clean --all �?tc qdisc del dev $iface root" >&2; exit 1; }
        echo "$iface" > "$SIDECAR"
