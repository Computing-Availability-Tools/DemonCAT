...
def provision(provs, ctx, iface):
    ...
    if "loop_device" in provs:
        if os.geteuid() == 0:
            sh("dmsetup ls --target error 2>/dev/null | awk '/^dcat-/{print $1}' | xargs -r -n1 dmsetup remove -f 2>/dev/null; dmsetup ls --target delay 2>/dev/null | awk '/^dcat-/{print $1}' | xargs -r -n1 dmsetup remove -f 2>/dev/null; true")
