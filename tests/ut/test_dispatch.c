...
static result_t *mock_cap(const char *cmd, const char *const *env) {
    ...
    for (int i = 0; env[i]; i++) {
        if (strncmp(env[i], "DCAT_OP=", 8) == 0)
            op = env[i] + 8;
        else if (strncmp(env[i], "DCAT_PARAM_CORES=", 17) == 0)
            cores = env[i] + 17;
    }
    g_cap_op = op;
    g_cap_cores = cores;
...
int test_dispatch_clean_no_match(void) {
    ...
    result_t *r = dispatch_route("rNET_delay", "clean", &c);
    ASSERT_INT_EQ(r->code, 0); /* idempotent: no active record = already clean */
    result_free(r);
    return 0;
}
...
int test_dispatch_clean_all_fans_out(void) {
    ...
    ASSERT_INT_EQ(r->code, 0);
    int nf = 0;
    const fault_def_t *lst = registry_list(&nf);
    int expect = 0;
    for (int i = 0; i < nf; i++)
        if (op_in_supported(lst[i].supported_ops, "clean")) expect++;
...
int test_dispatch_clean_all_marks_state_inactive(void) {
    ...
    params_t a;
    params_init(&a);
    params_set(&a, "cores", "0");
    dispatch_route("rCPU_overload", "inject", &a);
    params_t b;
    params_init(&b);
    params_set(&b, "iface", "eth0");
    params_set(&b, "delay_ms", "100");
...
int test_dispatch_clean_all_skips_reconcile_on_failure(void) {
    ...
    result_t *r = dispatch_clean_all();
    ASSERT_TRUE(r != NULL);
    ASSERT_INT_EQ(r->code, 1); /* --all best-effort 有失败 → code=1 */
