...
/* 动态插件接口（dlopen .so）�?so 须导�?dcat_plugin_get() 返回此结构指针�? * abi_version 必须等于 DCAT_PLUGIN_ABI_VERSION，否则加载器拒绝�?*/
typedef struct dcat_plugin_t {
    int abi_version;              /* 加载时校�?*/
    ...
    const char *clean_optional;  /* clean 可选参�?*/
    const char *query_required;  /* query 必填参数 */
    const char *query_optional;  /* query 可选参�?*/
