...
case "${DCAT_OP:-inject}" in
    inject)
        ...
        [ -z "$dev_id" ] && dev_id=0
        size_mb=${DCAT_PARAM_SIZE_MB:-2048}
        duration=${DCAT_PARAM_DURATION:-30}
        "$STRESS_BIN" hbm "$dev_id" "$duration" "$size_mb" >/dev/null 2>&1 &
        echo $! > "$SIDECAR"
        echo "HBM stress started on chip $chip (acl dev $dev_id, pid $!, ${size_mb}MB, ${duration}s)"
        ;;
    clean)
        if [ -f "$SIDECAR" ]; then
