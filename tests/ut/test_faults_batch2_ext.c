...
int main(void) {
    ...
    /* rCPU_quota (cores + quota_pct) */
    {
        ...
        r = dispatch_route("rCPU_quota", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rCPU_freq (cores,freq_mhz) */
    {
        ...
        r = dispatch_route("rCPU_freq", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rCPU_core_hang */
    {
        ...
        r = dispatch_route("rCPU_core_hang", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rDISK_part_full (path, optional size) */
    {
        ...
        r = dispatch_route("rDISK_part_full", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rDISK_inode_exhaust (path, optional count) */
    {
        ...
        r = dispatch_route("rDISK_inode_exhaust", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rDISK_io_delay (device,delay_ms) */
    {
        ...
        r = dispatch_route("rDISK_io_delay", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rDISK_io_error (device) */
    {
        ...
        r = dispatch_route("rDISK_io_error", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rNET_corrupt (iface,corrupt_pct) */
    {
        ...
        r = dispatch_route("rNET_corrupt", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rNET_conn_exhaust (target, optional count) */
    {
        ...
        r = dispatch_route("rNET_conn_exhaust", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rPROC_fork_bomb (count) */
    {
        ...
        r = dispatch_route("rPROC_fork_bomb", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rPROC_fd_exhaust (count) */
    {
        ...
        r = dispatch_route("rPROC_fd_exhaust", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    ...
    /* rNPU_pcie_down (npu_id,gen) */
    {
        ...
        check_param_env("npu_id", "0"); check_param_env("gen", "1"); result_free(r);
        r = dispatch_route("rNPU_pcie_down", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rNPU_aic_load (chip) */
    {
        ...
        r = dispatch_route("rNPU_aic_load", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rNPU_aiv_load (chip) */
    {
        ...
        check_param_env("chip", "0"); result_free(r);
        r = dispatch_route("rNPU_aiv_load", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
    /* rNPU_hbm_load (chip, size) */
    {
        ...
        r = dispatch_route("rNPU_hbm_load", "clean", &p);
        CK(r && r->code == 0);
        result_free(r);
    }
