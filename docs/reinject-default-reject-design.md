...
- **CNF 路径**(config 驱动 37 故障,`registry_find` 命中):apply。
...
- **资源键 = `f->clean_required` 各参数值**(grep 已验 37 条均纯资源标识:cores/device/iface/port/service/pid/chip*),零新增 config 字段。
...
- 插件 / legacy injector 路径未接入 reinject(CNF 路径覆盖 37 故障)。
