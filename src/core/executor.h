...
#include "types.h"
/* cnf 故障 inject/clean 路径：fork/exec+pipe 同步，可�?timer 超时；timeout_ms<=0 不超时�? * 成功返回 result_ok(op,uid,0,message=脚本 stdout 首行)；失败返�?result_err(code=1)�?*/
result_t *executor_run_fault(const fault_def_t *f, const char *op, const params_t *params, int timeout_ms);
/* cnf 故障 query 路径：设�?env �?system() 直�?stdout（继承终端），返回脚本退出码 */
