...
#include "types.h"
/* fault_def 版本预检（cnf 故障用）。返�?NULL=通过；非 NULL=失败 result_t(code)。fault=NULL �?code 4 */
result_t *precheck(const fault_def_t *f, const char *op, const params_t *params);
/* 通用预检函数（接受字符串，fault_def �?plugin 共用�?*/
...
const char *precheck_last_undeclared_param(void);  /* 返回上次 declared_params_only 检测到的首个未声明参数�?*/
#endif
