...
| 文件 | 说明 |
|---|---|
| `gen_cases.py` | 从故障目录 + 观测/边界/安全知识自动生成 `cases.csv`（329 条） |
| `cases.csv` | 用例表（329 条，8 类分类） |
...
|---|---|---|---|---|
| **FUNC** | `FUNC-` | ~150 | 37 故障 inject→verify→clean→query 全链路 + query\<uid\> confirmed + 插件 | 功能基线 |
| **BOUND** | `BOUND-` | ~40 | 每参数类型系统性覆盖（整数越界/空值/格式错误/枚举非法） | 边界值 |
