...
- **CNF 路径**（config 驱动的 60 条故障，`registry_find` 命中）：适用。
...
├── README.md
├── SPEC.md                     # 技术规格
├── User_Manual.md              # 用户手册（60 条故障 × 7 字段）
├── Release_Notes.md           # 版本发布记录
├── config/
│   └── demoncat.conf           # 故障目录配置（60 条故障声明）
├── docs/
│   ├── DESIGN.md               # 本文件：架构设计
│   ├── DemonCAT_Error_List.md  # 故障目录（60 条）
...
- **决策7（发布批次）**：v0.1 起步（核心框架 + 60 条故障），后续按需扩充（SPEC §8）。
